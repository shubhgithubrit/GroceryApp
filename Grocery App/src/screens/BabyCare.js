...
export default function BabyCare({ navigation }) {
  const [babyCare, setBabyCare] = useState([]);
  const [isLoading, setIsLoading] = useState(false);
  const [modalVisible, setModalVisible] = useState(false);
  const [input, setInput] = useState("");
  const [filterData, BabyCarefilter] = useState([]);
  ...
  const searchBabyCare = (textToSearch) => {
    BabyCarefilter(
      babyCare.filter((item) =>
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {isLoading ? (
        ...
        <FlatList
          data={
            filterData.length == 0 ? (input != "" ? [] : babyCare) : filterData
          }
