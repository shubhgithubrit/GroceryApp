...
export default function Masala({ navigation }) {
  const [val, setValue] = useState([]);
  const [isLoading, setIsLoading] = useState(false);
  const [modalVisible, setModalVisible] = useState(false);
  const [input, setInput] = useState("");
  const [filterData, setMasala] = useState([]);
  ...
  const searchMasala = (textToSearch) => {
    setMasala(
      val.filter((item) =>
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {isLoading ? (
        ...
        <FlatList
          data={
            filterData.length == 0 ? (input != "" ? [] : val) : filterData
          }
