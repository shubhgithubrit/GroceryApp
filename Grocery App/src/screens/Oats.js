...
export default function Oats({ navigation }) {
  ...
  const fetchOats = () => {
    ...
    database
      ...
      .then((item) => {
        ...
        setOats(users); // updating stateMessages
        setIsLoading(false);
      });
  };
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {isLoading ? (
        ...
        <FlatList
          data={
            filterData.length == 0 ? (input != "" ? [] : oats) : filterData
          }
          // data={oats}
