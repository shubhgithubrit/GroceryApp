...
export default function SoftDrink({ navigation }) {
  ...
  const fetchSoftDrink = () => {
    setIsLoading(true);
    database
      .ref("categories/Drink")
      .once("value")
      .then((item) => {
        let users = [];
  ...
  const searchSoftDrink = (textToSearch) => {
    setFilterData(
      softdrink.filter((item) =>
  ...
  return (
    <View style={{ flex: 1 }}>
      <CustomHeader
        ...
        val3={() => navigation.navigate('Home')}
      />
      <SearchBar
        placeholder="Search here"
        onChangeText={(text) => {
          setInput(text);
          searchSoftDrink(text);
        }}
        value={input}
      ...
      {isLoading ? (
        ...
        <FlatList
          data={
            filterData.length == 0 ? (input != "" ? [] : softdrink) : filterData
          }
